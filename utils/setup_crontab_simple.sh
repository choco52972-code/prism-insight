...
# 오전 9시 30분 - 오전 분석 (월-금)
30 9 * * 1-5 cd $PROJECT_DIR && $PYTHON_CMD stock_analysis_orchestrator.py --mode morning >> $PROJECT_DIR/logs/morning_$(date +\%Y\%m\%d).log 2>&1

# 오후 3시 40분 - 오후 분석 (월-금)
40 15 * * 1-5 cd $PROJECT_DIR && $PYTHON_CMD stock_analysis_orchestrator.py --mode afternoon >> $PROJECT_DIR/logs/afternoon_$(date +\%Y\%m\%d).log 2>&1
